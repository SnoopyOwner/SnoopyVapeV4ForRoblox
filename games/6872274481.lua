...
run(function()
	...
	KillauraVisualizer = Killaura:CreateToggle({
		...
		Function = function(callback)
			...
			KillauraVisualizer.Clean = cleanVisualizer
		end
	})
...
run(function()
	...
	vape.Components.HotbarList = function(optionsettings, children, api)
		...
		textbuttonicon.Image = getcustomasset('snoopyvape/assets/new/add.png')
			...
			local mounted = bedwars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						BackgroundTransparency = 1,
						Image = getcustomasset('snoopyvape/assets/new/blur.png'),
    ...
    Function = function(callback)
        if callback then
			...
			con = lplr.CharacterAdded:Connect(function(v)
				con:Disconnect();
				task.wait(0.2)
				tween:Create(v.PrimaryPart, TweenInfo.new(0.75), {CFrame = bed.Bed.CFrame + Vector3.new(0, 5, 0)}):Play();
			end);
        end
    end
})
			...
			if call then
				...
				antihit:Clean(runService.Heartbeat:Connect(function()
					if oldroot and oldroot.Parent and antihitcloned then
						...
						oldroot.CFrame = antihitting and CFrame.new(clone.CFrame.X, lastypos + 500, clone.CFrame.Z) or clone.CFrame
					end
				end))
				repeat
					if entitylib.isAlive then
